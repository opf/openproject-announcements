module OpenProject::Announcements
  class Engine < ::Rails::Engine
    ...
    initializer 'announcements.register_factories', after: 'factory_girl.set_factory_paths' do |_app|
      if defined?(FactoryGirl)
        FactoryGirl.definition_file_paths << File.expand_path(root.to_s + '/spec/factories')
      end
    end
    ...
    config.before_configuration do |app|
      ...
      app.config.paths['config/routes'].unshift File.join(File.dirname(__FILE__),
                                                          '..', '..', '..', 'config', 'routes.rb')
    end
